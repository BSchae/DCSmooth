...
This package is accompanying @SchaeferFeng2020 and @Schaefer2021. Estimation procedures for long-memory errors is based upon the findings of @Fengetal2020. Boundary modification procedures for local polynomial smoothing are considered by @FengSchaefer2021. For a more comprehensive and detailed description of the algorithms than in section 4, please refer to these papers.
...
* **IPI_options** Advanced options for tuning the parameters of the iterative plug-in algorithm of the bandwidth selection. These options include 2-element vectors for the inflation parameters (`infl_par`), the inflation exponents (`infl_exp`) and a boundary shrinking parameter for stabilized estimation of the neccessary derivatives (`delta`). Another option to further stabilize estimation of derivatives at the boundaries is the use of a constant estimation window at the boundaries setting the boolean variable `const_window` to `TRUE`. The default values for the IPI-options depend partly on the regression type selected and are given below.
...
* **Y** The matrix of simulated values with size $n_x \times n_t$ (determined by function arguments `n_x`, `n_t`). The matrix $Y$ is the lower left $n_x \times n_t$ submatrix of the actually simulated matrix $Y'$ of size $2 n_x \times 2 n_t$ to aviod effects from setting the initial values ("burn-in period").
...
* **innov** The matrix of estimated innovations from the Hannan-Rissane algorithm.
...
This is the default method, specification of options is not necessary.
```{r}
...
The double conditional smoothing [@FengPeitz2021] is a spatial smoothing technique which effectively reduces the twodimensional estimation to two onedimensional estimation procedures. The DCS is defined for kernel regression as well as for local polynomial regression.
...
$$
which is called exponential inflation method (EIM) [@BeranFeng2002a]. The values of $c_k$ are chosen on simulations, that of $alpha$ are subject to the derivative of interest.
