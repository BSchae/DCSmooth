...
| `returns.bmw`        |  5 minute returns of BMW AG from 2007 to 2010 |
| `returns.alv`        |  5 minute returns of Allianz SE from 2007 to 2010 |
| `volumes.bmw`        |  5 minute volumes of BMW AG from 2007 to 2010 |
| `volumes.bmw`        |  5 minute volumes of Allianz SE from 2007 to 2010 |
...
* **kernel_id** The identifier for the kernel to be assigned. It is a character string of the form `"X_abc"`, where `X` is specifies the type (`MW`, `M`, `T`), `a` i the kernel order $k$, `b` is the degree of smootheness $\mu$ and `c` is the order of derivative $\nu$. A list of currently useable kernel identifiers can be accessed with the function `kernel.list()`.
...
The environmental application data sets feature the temperature, wind speed and solar radiation surfaces of Nunn, CO (`temp.nunn`, `wind.nunn`, `sun.nunn`) and Yuma, AZ (`temp.yuma`, `wind.yuma`, `sun.yuma`). The observations are taken in 2020 in 5-minute intervals. The temperatures are given in *Celsius*, wind speed in *m/s* and solar radiation in *W/m^2*. All data sets consist therefore of 288 columns (the intraday observations) and 366 rows (the days). The data is taken from the U.S. Climate Reference Network database at [www.ncdc.noaa.gov](https://www.ncdc.noaa.gov/crn/qcdatasets.html). (see Diamond et. al. (2013), [doi:10.1175/BAMS-D-12-00170.1](https://journals.ametsoc.org/view/journals/bams/94/4/bams-d-12-00170.1.xml).
...
The double conditional smoothing (DCS, see @Feng2013) is a spatial smoothing technique which effectively reduces the twodimensional estimation to two one-dimensional estimation procedures. The DCS is defined for kernel regression as well as for local polynomial regression.
...
The main idea Hannan-Rissanen algorithm is to use a high-order SAR auxiliary model for initial estimation of the unobservable innovations sequence. Then, a linear regression model is applied, which yields the SARMA-parameters from the observations and estimated innovations.
