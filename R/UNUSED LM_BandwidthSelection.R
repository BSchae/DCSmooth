...
# ################################################################################
# 
# LM.bndw = function(Y, kernel_x, kernel_t, dcs_options, add_options)
# {
...
#     h_infl          = inflation.LP(h_opt_temp, c(n_x, n_x), dcs_options)  # TODO: change inflation method?
...
#     R_mat = Y - Y_smth
#     var_coef = var.coef.LM(R_mat, model_order = list(ar = c(0, 0), ma = c(0, 0)))
#     
#     # calculate optimal bandwidths for next step
#     h_opt = h.opt.LM(mxx, mtt, d_vec, var_coef, p_order, drv_vec, n_x, n_t, n_sub,
#                         kernel_x, kernel_t)
#     
#     # break condition
#     if( all(!is.nan(h_opt)) &&
